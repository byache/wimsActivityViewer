...
def computescore(listscoresnote,prepare,sn,st):
	...
	"""
	nbex=len(listscoresnote) #nbre d'exercices
	weights,requires,maxi,sev=prepare
	I0,I1,I2,Q,sommecoef,sommecoefQ = 0,0,0,0,0,0
	sommecoef2 = 0 #ce n'est pas indiqué dans l'aide de wims, mais pour i2, lorsqu'il n'y a pas eu d'essai sur l'exo, on ne compte pas cet exo (on ne met pas 0)
	#bizarrerie : pour i1, ce phénomène n'intervient pas alors que i1 ets supposé moins "laxiste" que i2 ?
	pasfait = 0
	for num in range(nbex):
		list = listscoresnote[num]
		if list == []:
			pasfait+=1
		#flash(list)
	...
	#flash(Q)
	I = indic[sev[2]]
	I = round(I,2) #encore un truc en peu bizarre : wims ne prend que deux décimales...
	Q = round(Q,2) #encore un truc en peu bizarre : wims ne prend que deux décimales...
	regle = sev[1]
	#flash('I')
	#flash(I)
	note = 0
